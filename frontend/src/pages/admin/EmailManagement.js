...
function TemplatesTab({
  ...
  return (
    <div className="grid grid-cols-12 gap-6">
      {/* Sidebar list */}
      <aside className="col-span-12 lg:col-span-3 bg-white rounded-lg border border-gray-200 overflow-hidden">
      ...
      <aside className="col-span-12 lg:col-span-3 space-y-4">
            ...
            <ul className="space-y-1.5" data-testid="email-tpl-vars">
              {Object.entries(variables).map(([k, desc]) => (
                <li key={k}>
                  ...
                  >
                    <code className="text-[#c9a84c] font-mono">{`{{${k}}}`}</code>
        ...
        <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
          ...
          <div className="p-3 bg-gray-100" style={{ maxHeight: 600, overflow: 'auto' }} data-testid="email-tpl-preview">
            {previewSubject && (
            ...
            {previewHtml ? (
              <iframe
                title="Email preview"
                srcDoc={previewHtml}
                style={{ width: '100%', minHeight: 480, border: 0, background: '#fff', borderRadius: 6 }}
              />
            ) : (
...
function BrandingTab({ branding, setBranding, saving, setSaving, onSaved }) {
  ...
  useEffect(() => {
    ...
    return () => clearTimeout(handle);
  }, [branding?.logo_url, branding?.primary_color, branding?.button_color, branding?.button_text_color, branding?.footer_text, branding?.social_links?.length]);
