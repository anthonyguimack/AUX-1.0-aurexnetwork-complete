...
import { Download, Upload, Loader2, Check, AlertTriangle, Database, FileJson, RefreshCw, Shield } from 'lucide-react';
...
export default function BackupManager() {
  ...
  const handleExport = async () => {
    ...
    try {
      const cols = [...selectedExport].join(',');
      const res = await adminAPI.exportContent(cols);
      const data = res.data;
    ...
    reader.onload = (ev) => {
      try {
        ...
        for (const c of collections) {
          counts[c] = Array.isArray(data[c]) ? data[c].length : (data[c] ? 1 : 0);
        }
  ...
  const handleImport = async () => {
    ...
    setImportResult(null);
    try {
      const payload = { ...importPreview.raw, _mode: importMode };
      const res = await adminAPI.importContent(payload);
    ...
    <div data-testid="backup-manager">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-slate-400 mt-1">Export your CMS content as JSON or restore from a backup file</p>
        </div>
                ...
                <div className="space-y-1">
                  {Object.entries(importResult.results || {}).map(([col, r]) => (
                    <div key={col} className={`flex items-center justify-between px-2 py-1 rounded text-xs ${r.status === 'ok' ? 'bg-white text-slate-600' : 'bg-red-50 text-red-600'}`}>
                      ...
                      {r.status === 'ok' ? (
                        <span className="font-mono text-green-600">{r.count} items</span>
                      ) : (
