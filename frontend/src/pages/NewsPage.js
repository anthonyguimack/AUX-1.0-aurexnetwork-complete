...
export default function NewsPage() {
  ...
  const categories = [...new Set(data.posts.map(p => p.category).filter(Boolean))];
          ...
          {data.posts.map(post => (
            <Link key={post.id} to={`/news/${post.slug}`} className="group bg-white rounded-sm border border-slate-100 overflow-hidden hover:shadow-xl transition-all" data-testid={`news-card-${post.slug}`}>
              ...
              <div className="p-6">
                ...
                <h3 className="text-lg font-semibold text-[#1a2332] group-hover:text-[#0D9488] transition-colors" style={{ fontFamily: 'Playfair Display, serif' }}>{post.title}</h3>
                <p className="text-sm text-slate-500 mt-2 line-clamp-3">{post.summary}</p>
