...
import { LogIn, Mail, Lock, Loader2, ArrowLeft, KeyRound } from 'lucide-react';
...
export default function LoginModal({ open, onClose }) {
  ...
  const handleLogin = async (e) => {
    ...
    setError('');
    try {
      const loginType = tab === 'admin' ? 'admin' : 'user';
        ...
        <div className="p-6">
          {view === 'login' && (
            <>
              ...
              <div className="flex mb-4 bg-slate-100 rounded-sm p-0.5">
                <button onClick={() => { setTab('user'); setError(''); }} className={`flex-1 py-2 text-sm font-medium rounded-sm transition-colors ${tab === 'user' ? 'bg-white shadow-sm' : 'text-slate-500'}`} style={tab === 'user' ? { color: 'var(--color-primary, #1a2332)' } : {}} data-testid="login-tab-user">User</button>
                <button onClick={() => { setTab('admin'); setError(''); }} className={`flex-1 py-2 text-sm font-medium rounded-sm transition-colors ${tab === 'admin' ? 'bg-white shadow-sm' : 'text-slate-500'}`} style={tab === 'admin' ? { color: 'var(--color-primary, #1a2332)' } : {}} data-testid="login-tab-admin">Administrator</button>
              </div>
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div>
                  ...
                  <div className="relative mt-1">
                    ...
                    <Input type="email" placeholder={tab === 'admin' ? 'admin@consultant.com' : 'user@example.com'} value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10" required data-testid="login-email-input" />
                  </div>
                </div>
                <div>
                ...
                <button type="submit" disabled={loading} className="w-full py-2.5 rounded-sm font-medium transition-colors flex items-center justify-center gap-2 disabled:opacity-50" style={{ backgroundColor: 'var(--color-button-bg, #1a2332)', color: 'var(--color-button-text, #fff)' }} data-testid="login-submit-btn">
                  ...
                  {loading ? 'Signing in...' : `Sign In as ${tab === 'admin' ? 'Admin' : 'User'}`}
                </button>
              </form>
              {tab === 'user' && (
                <>
